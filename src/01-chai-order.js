...
export function shoutChaiOrder(order) {
  if (!(typeof order ==="string") || order.trim().length<1) return "";
...
export function hasSpecialIngredient(order, ingredient) {
  if (!(typeof order==="string") || !(typeof ingredient==="string") || order.trim().length<1 || ingredient.trim().length<1) return false;
  return (order.trim().toLowerCase().includes(ingredient.trim().toLowerCase())) ? true : false; 
}

export function getFirstAndLastChar(order) {
  if (!(typeof order ==="string") || order.trim().length<1) return null;
