...
export function findPassenger(passengers, name) {
  if (!(Array.isArray(passengers)) || !name || name=="" || !(typeof name==='string')) return undefined;
  return passengers.find(p => p.name.toLowerCase()==name.toLowerCase());
}

export function getPassengerIndex(passengers, name) {
  if (!Array.isArray(passengers) || !name || name==null || !(typeof name==='string')) return -1;
...
export function areAllConfirmed(passengers) {
  if (!Array.isArray(passengers) || passengers.length<1) return false;
